...
## [2026-07-23] ESP32-S3 独立控制压缩机档位、风扇开关和水泵开关

**背景**：用户要求S3的档位和开关只作用于压缩机，风扇与水泵各自只保留独立开关，并将开机默认状态改为全部关闭。
**决策**：保留 `0x20 SET_GEAR` 作为压缩机命令，档位1..10线性映射为10%..100% PWM；新增 `0x21 SET_FAN` 和 `0x22 SET_PUMP`，payload均为单字节开关。开机显式执行 Compressor Stop、PC10 LOW、PC11 LOW。压缩机运行中换挡调用 SetDuty，只有OFF→ON才调用Start。水泵驱动改为纯GPIO开关，原2kHz软PWM回调保留为空操作以兼容现有中断调用。
**原因**：三个执行器解耦后，S3可独立管理制冷链路；运行中换挡不重启压缩机，也消除了重复ON反复重启的问题；水泵纯开关符合最新硬件控制要求。
**排除方案**：不再让 `SET_GEAR` 联动风扇和水泵；不保留水泵30%/100Hz供电PWM；不开机自动启动任何执行器。仓库没有S3端源码，因此本轮只实现STM32协议接收端并固化命令号。

---

## [2026-07-23] 水泵开关命令启停30%软件PWM而非全速供电

**背景**：用户澄清“水泵只有开关”是指S3只提供启停控制，水泵开启后的驱动仍需保持30%软件PWM，不能持续全速。
**决策**：`SET_PUMP(0x22)` ON调用 `PowerCtrl_EnablePump(1)` 启用PC11 100Hz/30%软PWM，OFF关闭PWM并保持PC11 LOW；默认占空比30%。取消旧版1秒100%启动阶段，开启后从第一周期即按30%运行。
**原因**：保持S3接口简单的同时满足水泵降速要求；取消全速启动严格符合“不能是全速”，并避免每次开关产生1秒满功率冲击。
**排除方案**：不采用PC11持续HIGH纯开关；不恢复1秒满功率启动；压缩机0x20和风扇0x21的独立控制保持不变。

---

## [2026-07-23] S3独立控制与开机全关版本烧录验证

**背景**：完成0x20/0x21/0x22独立控制和水泵30% PWM澄清后，需要确认App A烧录完整性与开机默认安全状态。
**决策**：确认DBGMCU ID=`0x10076413`、BKP0R=`0xA0A0A0A0`后仅烧录App A；复位后1.5s按最新ELF分别解析诊断变量并读取GPIO/TIM2寄存器。
**原因**：烧录明确显示`Programming Finished`、`Verified OK`且退出码为0；运行态 compressor_on=0、gear=0、duty=0、STOP=1、TIM2 CCR1=0，GPIOA ODR=0、GPIOC ODR=0，证明压缩机、风扇和水泵开机全关。水泵duty预设为30%，但output=0、startup_ticks=0，未发生全速或PWM输出。
**排除方案**：未修改bootloader/params或执行mass erase；未通过SWD强制注入ON命令，避免未经用户要求启动机械负载，0x20/0x21/0x22动作保留给S3联调验证。

---

## [2026-07-23] S3实际仅有0x20命令并用on联动三个执行器

**背景**：用户反馈三个执行器不可控并澄清S3没有独立风扇/水泵命令。SWD与UART原始环形缓冲确认通信正常，当前实际命令为gear=10、on=1；STM32因此将压缩机输出到100%，而上一版因错误等待0x21/0x22导致风扇和水泵保持关闭。
**决策**：删除STM32端0x21/0x22及对应命令结构。唯一 `SET_GEAR(0x20)` 的gear只改变压缩机PWM档位，on同时控制压缩机、风扇PC10和水泵30%软件PWM；on=0立即停止并关闭三者。开机默认全关保持不变。
**原因**：现场有效帧计数增长且XOR错误为0，原始帧证明S3只发送0x20。控制输出与收到的gear=10/on=1完全一致，根因是协议拆分假设错误，而非UART或GPIO故障。
**排除方案**：不要求S3新增不存在的0x21/0x22；不让gear影响风扇或水泵速度；保留压缩机运行中换挡只SetDuty、不重复Start的去重逻辑。

---

## [2026-07-23] PA15 与 PB14 在主控板 BEEP_CTRL 网络硬短接

**背景**：现场将 PA15/TIM2_CH1 配置为高阻后仍测到 3.3V，且驱动板外置上拉电阻已拆除，需要沿原理图网表确认电压来源。
**决策**：暂停继续把 PA15 当作独立压缩机 PWM 使用。主控板 2026-04-28 网表明确显示 `BEEP_CTRL ; FPC2.6 R16.1 TP4.1 U9.53 U9.77`，其中 U9.53=PB14、U9.77=PA15；顶部驱动板网表又显示 FPC1.6 属于 `FAN_PWM_CTRL`。现场诊断时已暂停 MCU，并同时将 PA15、PB14 配为无上下拉高阻，等待分段电压测量。
**原因**：PA15 与 PB14 是 PCB 同一网络，拆除驱动板上拉不能解除 PB14/TIM1_CH2N 或 FPC 对该线的影响；两个 MCU 推挽外设同时配置还会产生电平争用和过流风险。
**排除方案**：不再将 PA15 的 3.3V 归因于未确认的外部上拉；不允许只靠固件关闭蜂鸣器来宣称 PA15 已独立。最终方案必须物理断开该网络或选择未短接的 PWM 引脚，并用断线两侧电压/通断测量验证。

---

## [2026-07-23] 纠正 PA15/PB14 网表误连结论

**背景**：主控板网表将 PA15 与 PB14 同列在 BEEP_CTRL，但用户依据实际 PCB 明确确认两者没有物理连接。
**决策**：以实板为准撤销“PA15/PB14 硬短接”的当前硬件判断，将该网表条目标记为原理图/网表错误。现场 MCU 保持暂停，PA15 改为数字输入内部下拉，TIM2_CH1 CC1E=0、CCR1=0；SWD 读取 GPIOA IDR bit15=0。
**原因**：内部下拉启用后 MCU 实际 PA15 输入为 LOW，与测试点所测3.3V不一致，说明所测节点尚未证明与 U9.77 导通；不能再用错误网表推断电压来源。
**排除方案**：不修改历史决策记录，但后续不得引用上一条硬短接判断作为实板事实；必须断电测 U9.77 到目标测试点的电阻，并分别测 MCU 焊盘侧与 FPC/驱动侧电压。

---

## [2026-07-23] 压缩机 PWM 从 PA15 迁移到 PA0

**背景**：用户确认硬件已修改，PA0 接替此前 PA15 的压缩机 PWM 功能，PA0 不再承担 KEYWAKE。
**决策**：继续复用 TIM2_CH1/AF1、5kHz、PWM1、高电平有效和原档位占空比逻辑，仅将 App 的 TIM2_CH1 GPIO 复用从 PA15 改为 PA0；删除 PA0 KEYWAKE 初始化和读取接口。App 在定时器初始化前先将 PA0 推挽拉低，Bootloader 也保持 PA0 LOW；PA15 在 App 与 Bootloader 均设置为无上下拉模拟模式。
**原因**：PA0 支持 TIM2_CH1 AF1，可在不改变定时器、协议和压缩机控制状态机的情况下完成引脚迁移；Bootloader/App 的显式安全态可避免复位和槽位选择窗口出现高电平脉冲。
**排除方案**：不改用 TIM5_CH1，避免新增定时器实例和控制路径；不保留 KEYWAKE 兼容读取，防止业务代码读取正在输出 PWM 的 PA0；不让 PA15保持复用或推挽输出。

**构建验证**：Bootloader、App A、App B 三个 preset 均链接成功；App A/B text=28096、data=168、bss=5608，Flash占用28272B/128KB，RAM占用5768B/128KB。仅出现工程既有的newlib未实现 `_close/_lseek/_read/_write` 与RWX LOAD segment链接警告，无编译或链接错误。本轮未烧录。

---

## [2026-07-23] 恢复 PA15 蜂鸣器并将压缩机 PWM 改用 TIM5_CH1

**背景**：压缩机PWM已迁移到PA0后，用户要求恢复蜂鸣器；实板蜂鸣器控制需要PA15。PA0与PA15都可映射TIM2_CH1，但同一个定时器通道不能同时输出固定5kHz压缩机PWM和可变频率蜂鸣器音调。
**决策**：PA15恢复为TIM2_CH1/AF1无源蜂鸣器，使用通用PWM CH1；PA0压缩机PWM改为TIM5_CH1/AF2，保持5kHz、PWM1、高电平有效及原占空比逻辑。废弃PB14/TIM1_CH2N蜂鸣器路径。Bootloader在启动期间将PA0和PA15都推挽拉低。
**原因**：TIM5_CH1在PA0上可独立于PA15/TIM2_CH1运行，两个波形的频率、占空比和启停互不干扰；同时保留PA15蜂鸣器的硬件连线。
**排除方案**：不让PA0/PA15同时使用TIM2_CH1，因为它们只能镜像同一波形；不使用软件GPIO翻转产生蜂鸣器，避免新增高频中断和音调抖动；不继续使用PB14，因用户确认其与PA15实板不相连。

---

## [2026-07-23] 取消恢复蜂鸣器，保持 PA0/TIM2_CH1 压缩机 PWM

**背景**：提出PA15蜂鸣器/TIM5压缩机方案后，用户明确要求暂时不要蜂鸣器，并询问PA15蜂鸣器来源。
**决策**：撤销上一条尚未实施的迁移方案，不修改压缩机定时器：PA0继续使用TIM2_CH1/AF1输出5kHz PWM，PA15继续保持无上下拉高阻。App不再初始化PB14/TIM1_CH2N蜂鸣器，也不播放开机音或按键提示音。
**原因**：旧版PA15蜂鸣器来源是历史BEEP_CTRL连线与V3固件，不是当前必需功能；暂时关闭蜂鸣器可避免为未确认的实板蜂鸣器连线重新分配定时器。
**排除方案**：不实施PA0/TIM5_CH1压缩机迁移；不恢复PA15/TIM2_CH1蜂鸣器；不保留PB14定时器输出处于已初始化状态。

---

## [2026-07-23] 修正：蜂鸣器保持原PB14/TIM1实现，与PA0/PA15迁移无关

**背景**：用户澄清蜂鸣器已有独立原有代码和硬件路径，与PA15、PA0均无关；此前根据旧版记录推断PA15蜂鸣器并尝试关闭现有蜂鸣器属于错误操作。
**决策**：完整恢复并保持原有PB14/TIM1_CH2N蜂鸣器初始化、开机音和按键提示音，不修改 `buzzer.c/.h`。本次硬件变更仅包含压缩机PWM从PA15迁移到PA0/TIM2_CH1、PA0取消KEYWAKE、PA15保持高阻。
**原因**：用户提供的当前实板事实优先于旧版V3历史记录；蜂鸣器已有独立定时器TIM1，不与压缩机TIM2资源冲突。
